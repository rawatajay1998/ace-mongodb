...
const ContactEnquiries = () => {
  const [contactData, setContactData] = useState<ContactEnquiry[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get("/api/enquiry/contact", {
        params: {
          page,
          pageSize,
          searchName,
  ...
  return (
    <div className="card">
      <h2 className="main_title">Contact Page Enquiries</h2>
      <Table
        columns={columns}
